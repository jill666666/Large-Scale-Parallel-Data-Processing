...
object RDDAggregateMain {
  
  def main(args: Array[String]) {
    val logger: org.apache.log4j.Logger = LogManager.getRootLogger
    if (args.length != 2) {
      logger.error("Usage:\nfc.RDDAggregateMain <input dir> <output dir>")
      System.exit(1)
    }
...
//    try { hdfs.delete(new org.apache.hadoop.fs.Path(args(1)), true) } catch { case _: Throwable => {} }
    ...
    val initialCount = 0
    val sumFollowerCounts = (c1: Int, c2: Int) => c1 + c2 // set rule that aggregates int values
    val sumPartitionCounts = (p1: Int, p2: Int) => p1 + p2 // set rule that aggregates int values
    ...
    val counts = textFile
                 ...
                 .aggregateByKey(initialCount)(sumFollowerCounts, sumPartitionCounts)
                  // sum up each of the follower counts and aggregate the partition counts
